...
class LIFLSharedMemoryBackend(AbstractBackend):
    ...
    def leave(self, channel) -> None:
        ...
        # Best-effort cleanup; ignore missing segments.
        for end in channel.all_ends():
            try:
                ...
                logger.debug(f"SharedMemory segment {end} already removed")
    ...
    async def _unicast_task(self, channel, end_id):
        ...
        while True:
            ...
            if data == EMPTY_PAYLOAD:
                txq.task_done()
                logger.debug("unicast task got an empty msg from queue")
                break
    ...
    def send_data(self, other: str, ch_name: str, data: bytes) -> None:
        ...
        skmsg_md_bytes = b"".join([
            next_fn_id_int.to_bytes(4, byteorder="little"),
    ...
    def _rx_task(self) -> None:
        ...
        while True:
            try:
                ...
                # Connection closed
                if not skmsg_md:
                    logger.warning(f"{self._id} sockmap socket closed by peer")
                    break

                logger.debug(f"{self._id} received request: {skmsg_md}")

                if len(skmsg_md) < 4:
                    logger.warning(f"{self._id} received too short skmsg_md: {skmsg_md}")
                    continue
                ...
                if truncated_target_end_id != truncated_self_id:
                    logger.warning(
                        f"{self._id} (truncated_self_id={truncated_self_id}) "
                        f"received unexpected msg! target_end_id: {truncated_target_end_id}"
                    )
                ...
                ex_name = type(ex).__name__
                logger.error(f"{self._id} error in _rx_task: {ex_name}: {ex}")
                # avoid tight error loop
                time.sleep(0.1)
                break

        logger.info(f"{self._id} _rx_task terminated")
        ...
        while True:
            ...
            try:
                ...
                if msg.channel_name in self._channels:
                    ...
                    logger.warning(
                        f"{self._id} received msg for unknown channel {msg.channel_name}"
                    )
            finally:
                # ensure queue bookkeeping is correct
                self._rx_queue.task_done()
    ...
    def get_data(self, other, msg_size) -> Union[bytes, None]:
        ...
        shm_name = other + "-" + self._id
        try:
            read_buf = shared_memory.SharedMemory(shm_name)
        except FileNotFoundError:
            logger.error(f"{self._id} SharedMemory segment {shm_name} not found")
            return None
    ...
    def set_data(self, data: bytes, other: str) -> None:
        ...
        try:
            if key not in self._is_shm_buf_created:
                ...
                self._is_shm_buf_created[key] = True
            else:
                wrt_buf = shared_memory.SharedMemory(name=shm_name)
                # If new data is larger than existing buffer, recreate it.
                if len(data) > len(wrt_buf.buf):
                    wrt_buf.close()
                    wrt_buf.unlink()
                    wrt_buf = shared_memory.SharedMemory(
                        name=shm_name, create=True, size=len(data)
            ...
            # we intentionally do not unlink here; receiver still needs it
            wrt_buf.close()
    ...
    async def handle(self, msg: msg_pb2.Data, channel: Channel) -> None:
        ...
        if payload is None:
            logger.error(
                f"{self._id} failed to get payload from shared memory for end {msg.end_id}"
            )
            return
